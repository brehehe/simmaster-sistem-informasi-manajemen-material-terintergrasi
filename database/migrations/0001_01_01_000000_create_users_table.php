...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->timestamp('email_verified_at')->nullable()->index(); // tambah index
            ...
            $table->index('deleted_at'); // index untuk soft delete
        });

        Schema::create('password_reset_tokens', function (Blueprint $table) {
            $table->string('email')->primary();
            $table->string('token');
            $table->timestamp('created_at')->nullable(); // tambah index

            $table->index('created_at'); // tambahan agar lebih cepat dihapus / filter
        });
