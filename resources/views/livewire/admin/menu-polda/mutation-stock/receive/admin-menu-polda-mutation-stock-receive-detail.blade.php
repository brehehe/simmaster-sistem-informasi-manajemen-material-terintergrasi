        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <div class="flex items-center gap-3 mb-2">
                    <a href="{{ route('menu-polda.mutation-stock.receive') }}" wire:navigate
                    ...
                    <h1 class="text-3xl font-bold text-purple-600">Detail Mutasi Stock</h1>
                </div>
                <p class="text-gray-500 ml-14">Review dan konfirmasi penerimaan mutasi stock</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden mb-6">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Kode Mutasi</label>
                    <p class="text-lg font-bold text-purple-600">{{ $mutation->code }}</p>
                </div>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Tanggal Mutasi</label>
                    <p class="text-lg font-semibold text-gray-800">
                        {{ \Carbon\Carbon::parse($mutation->mutation_date)->format('d F Y') }}
                    </p>
                </div>
                ...
                <div>
                    ...
                    <p class="text-lg font-semibold text-gray-800">
                        {{ $mutation->sent_at ? \Carbon\Carbon::parse($mutation->sent_at)->format('d F Y, H:i') : '-' }}
                    </p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Dari</label>
                    <p class="text-lg font-semibold text-gray-800">{{ $mutation->getSenderName() }}</p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-500 mb-1">Ke</label>
                    <p class="text-lg font-semibold text-gray-800">{{ $mutation->getReceiverName() }}</p>
                </div>

                @if ($mutation->notes)
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-gray-500 mb-1">Catatan</label>
                        <p class="text-gray-700 bg-gray-50 p-3 rounded-lg">{{ $mutation->notes }}</p>
                    </div>
                @endif
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden mb-6">
        <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-orange-50 to-amber-50/50">
            <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                ...
                Detail Stock ({{ $mutation->mutationStockDetails->count() }} Items)
            </h2>
        </div>

        <div class="p-6">
            <div class="space-y-4">
                @foreach ($mutation->mutationStockDetails as $index => $detail)
                    <div
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-semibold text-gray-800">{{ $detail?->typeDetail?->name ?? '-' }}
                                </p>
                            </div>
            ...
            <div class="mt-6 p-4 rounded-xl bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-100">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-2xl font-bold text-purple-600">
                        {{ number_format($mutation->mutationStockDetails->sum('quantity'), 0) }} unit
                    </span>
                </div>
            </div>
        </div>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-end gap-3">
        ...
        @else
            <div
                ...
                ✅ Stock sudah diterima pada
                {{ $mutation->received_at ? \Carbon\Carbon::parse($mutation->received_at)->format('d F Y, H:i') : '-' }}
            </div>
        @endif
    </div>
</div>
