<div>
    ...
    <div class="mb-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-gray-500 mt-1">Monitoring pengeluaran barang dari gudang</p>
            </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-gradient-to-br from-rose-500 to-red-600 rounded-2xl p-5 text-white shadow-lg shadow-rose-500/30">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-rose-100 text-sm">Total Transaksi Keluar</p>
                    <p class="text-3xl font-bold mt-1">{{ count($this->stockOuts) }}</p>
                </div>
        ...
        <div
            ...
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-amber-100 text-sm">Total Barang Keluar</p>
                    <p class="text-3xl font-bold mt-1">{{ number_format(collect($this->stockOuts)->sum('jumlah')) }}</p>
                </div>
                <div class="bg-white/20 rounded-xl p-3">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
        ...
        <div
            class="bg-gradient-to-br from-green-500 to-emerald-500 rounded-2xl p-5 text-white shadow-lg shadow-green-500/30">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-green-100 text-sm">Disetujui</p>
                    <p class="text-3xl font-bold mt-1">
                        {{ collect($this->stockOuts)->where('status', 'Disetujui')->count() }}</p>
                </div>
                <div class="bg-white/20 rounded-xl p-3">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
        ...
        <div
            ...
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-gray-100 text-sm">Pending/Ditolak</p>
                    <p class="text-3xl font-bold mt-1">
                        {{ collect($this->stockOuts)->whereIn('status', ['Pending', 'Ditolak'])->count() }}</p>
                </div>
                <div class="bg-white/20 rounded-xl p-3">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
            ...
            <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                ...
                <div class="relative w-full md:w-80">
                    <input wire:model.live.debounce.300ms="search" type="text"
                        placeholder="Cari no dokumen, barang atau tujuan..."
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Pemohon
                        </th>
                        <th class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">
                            Status</th>
                    </tr>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($this->stockOuts as $index => $item)
                        <tr class="hover:bg-rose-50/50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-rose-100 text-rose-700">
                                    {{ $item['no_dokumen'] }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                        ...
                        <tr>
                            <td colspan="9" class="p-10 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 text-lg font-medium">Tidak ada data barang keluar</p>
                                    <p class="text-gray-400 text-sm mt-1">Data barang keluar tidak ditemukan</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
            <div class="text-sm text-gray-600">
                Menampilkan <span class="font-semibold">{{ count($this->stockOuts) }}</span> hasil
            </div>
        </div>
    </div>
</div>
