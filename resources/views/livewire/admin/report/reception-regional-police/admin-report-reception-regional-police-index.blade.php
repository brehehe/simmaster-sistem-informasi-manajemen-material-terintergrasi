<div>
    <div class="mb-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-teal-600">Laporan Penerimaan Polda</h1>
                <p class="text-gray-500 mt-1">Monitoring penerimaan barang dari supplier ke Polda</p>
            </div>
            <div class="flex gap-2">
                <button onclick="exportToExcel('receptionPoldaTable', 'laporan-penerimaan-polda')"
                ...
                <button onclick="exportToPDF({ tableId: 'receptionPoldaTable', title: 'Laporan Penerimaan Polda', filename: 'laporan-penerimaan-polda', summaryCards: [{ label: 'Total', value: '{{ $this->totalReceptions }}' }, { label: 'Total Item', value: '{{ number_format($this->totalItems) }}' }] })"
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div class="bg-gradient-to-br from-teal-500 to-green-600 rounded-2xl p-5 text-white shadow-lg shadow-teal-500/30">
        ...
        <div class="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl p-5 text-white shadow-lg shadow-blue-500/30">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-blue-100 text-sm">Total Item Diterima</p>
                    <p class="text-3xl font-bold mt-1">{{ number_format($this->totalItems) }}</p>
                </div>
                <div class="bg-white/20 rounded-xl p-3">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="bg-gradient-to-br from-purple-500 to-pink-600 rounded-2xl p-5 text-white shadow-lg shadow-purple-500/30">
            <div class="flex items-center justify-between">
                <div>
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
        <div class="p-4 border-b border-gray-100">
            <div class="flex flex-col md:flex-row items-center justify-between gap-4">
                <div class="flex flex-wrap items-center gap-4">
                    ...
                    <select wire:model.live="filterPolda" class="px-3 py-2 rounded-lg border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all duration-200 bg-gray-50 focus:bg-white text-sm">
                ...
                <div class="relative w-full md:w-80">
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari kode atau nama penerimaan..."
        ...
        <div class="overflow-x-auto">
            <table class="w-full" id="receptionPoldaTable">
                <thead>
                    <tr class="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Nama Penerimaan</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($receptions as $index => $reception)
                        <tr class="hover:bg-teal-50/50 transition-colors duration-150">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $receptions->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-teal-100 text-teal-700">
                                    {{ $reception->code }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                {{ $reception->date ? \Carbon\Carbon::parse($reception->date)->format('d M Y') : '-' }}
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-900">{{ $reception->name ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-medium text-gray-900">{{ $reception->regionalPolice->name ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold bg-blue-100 text-blue-700">
                                    {{ number_format($reception->receptionDetails->sum('quantity')) }} unit
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $reception->description ?? '-' }}</td>
                        </tr>
                    @empty
                        <tr>
