<div>
    ...
    <div class="mb-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-blue-600">Manajemen Detail Tipe</h1>
                <p class="text-gray-500 mt-1">Kelola data detail tipe produk</p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
        <div class="p-4 border-b border-gray-100">
            <div class="flex items-center justify-between gap-4">
                ...
                <div class="relative w-80">
                    <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari nama atau tipe..."
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($typeDetails as $index => $detail)
                        <tr class="hover:bg-blue-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $detail->created_at->format('d M Y, H:i') }}
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="p-10 text-center">
                                <p class="text-gray-500">Tidak ada data detail tipe</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if ($typeDetails->hasPages())
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <!-- Info Text -->
                    <div class="text-sm text-gray-600">
                        Menampilkan <span class="font-semibold">{{ $typeDetails->firstItem() }}</span>
                        sampai <span class="font-semibold">{{ $typeDetails->lastItem() }}</span>
                        dari <span class="font-semibold">{{ $typeDetails->total() }}</span> hasil
                    </div>

                    <!-- Pagination Numbers -->
                    <div class="flex items-center gap-1">
                        {{-- Previous Button --}}
                        @if ($typeDetails->onFirstPage())
                        ...
                        {{-- Page Numbers --}}
                        @php
                            $currentPage = $typeDetails->currentPage();
                            $lastPage = $typeDetails->lastPage();
        ...
        @else
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div class="text-sm text-gray-600">
                    Menampilkan <span class="font-semibold">{{ $typeDetails->count() }}</span> hasil
                </div>
            </div>
        @endif
    </div>

    @include('livewire.admin.master.type-detail.admin-master-type-detail-modal')
    ...
    @if ($showDeleteModal)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-md p-6 bg-white shadow-2xl rounded-2xl text-center">
                ...
                <h3 class="text-xl font-bold text-gray-900 mb-2">Hapus Detail Tipe</h3>
                <p class="text-gray-500 mb-6">Apakah Anda yakin ingin menghapus detail tipe ini?</p>
