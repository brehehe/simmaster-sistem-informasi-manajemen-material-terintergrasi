<div>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <div class="flex items-center gap-2 text-sm text-gray-500 mb-2">
                    <a href="{{ route('master.type-detail') }}" class="hover:text-blue-600">Manajemen Detail Tipe</a>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                    </svg>
                    <a href="{{ route('master.type-detail.service', ['type_detail_id' => $typeDetail->id]) }}" class="hover:text-blue-600">{{ $typeDetail->name }}</a>
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                    </svg>
                    <span class="font-medium text-blue-600">{{ $service->name }}</span>
                </div>
                <h1 class="text-3xl font-bold text-blue-600">Manajemen Service Detail</h1>
                <p class="text-gray-500 mt-1">Kelola data detail untuk service {{ $service->name }} (Detail Tipe: {{ $typeDetail->name }})</p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($serviceDetails as $index => $detail)
                        <tr class="hover:bg-blue-50/50 transition-colors">
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $serviceDetails->firstItem() + $index }}</td>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $detail->description ?? '-' }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="p-10 text-center">
                                <p class="text-gray-500">Tidak ada data service detail</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
            ...
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="text-sm text-gray-600">
                        Menampilkan <span class="font-semibold">{{ $serviceDetails->firstItem() }}</span>
                        sampai <span class="font-semibold">{{ $serviceDetails->lastItem() }}</span>
                        dari <span class="font-semibold">{{ $serviceDetails->total() }}</span> hasil
                    </div>
                    <div class="flex items-center gap-1">
                        @if ($serviceDetails->onFirstPage())
                        ...
                        @php
                            $currentPage = $serviceDetails->currentPage();
                            $lastPage = $serviceDetails->lastPage();
        ...
        @else
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div class="text-sm text-gray-600">
                    Menampilkan <span class="font-semibold">{{ $serviceDetails->count() }}</span> hasil
                </div>
            </div>
        @endif
    </div>

    @include('livewire.admin.master.type-detail.service.service-detail.admin-master-type-detail-service-detail-modal')
