        ...
        <div class="relative w-full max-w-xl bg-white shadow-2xl rounded-2xl overflow-hidden">
            <div class="bg-gradient-to-r from-blue-600 to-cyan-500 px-5 py-3">
                <div class="flex items-center justify-between">
                    <h3 class="text-lg font-bold text-white">{{ $isEditMode ? 'Edit Service Detail' : 'Tambah Service Detail Baru' }}</h3>
            ...
            <form wire:submit="save" class="p-5 space-y-4">
                <div>
                    ...
                    <input type="text" wire:model="name"
                        ...
                        placeholder="Nama service detail">
                    @error('name')
                ...
                <div>
                    ...
                    <input type="text" wire:model="price"
                        class="w-full px-3 py-2 text-sm rounded-lg border {{ $errors->has('price') ? 'border-red-300' : 'border-gray-200' }} focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 bg-gray-50"
                        placeholder="Harga service detail">
                    @error('price')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>
