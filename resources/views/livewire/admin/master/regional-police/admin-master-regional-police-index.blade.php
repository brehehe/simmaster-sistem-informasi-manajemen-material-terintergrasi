<div>
    ...
    <div class="mb-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-gray-500 mt-1">Kelola data Kepolisian Daerah</p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($regionalPolices as $index => $regionalPolice)
                        <tr class="hover:bg-blue-50/50 transition-colors duration-150">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                {{ $regionalPolices->firstItem() + $index }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="font-medium text-gray-900">{{ $regionalPolice->name }}</div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">
                                <div class="max-w-xs truncate">{{ $regionalPolice->description ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700">
                                    {{ $regionalPolice->police_stations_count }} Polres
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if ($regionalPolice->is_active)
                                    <span
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                {{ $regionalPolice->created_at->format('d M Y, H:i') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button wire:click="openEditModal('{{ $regionalPolice->id }}')"
                                    ...
                                    <button wire:click="openDeleteModal('{{ $regionalPolice->id }}')"
                        ...
                        <tr>
                            <td colspan="7" class="p-10 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 text-lg font-medium">Tidak ada data Polda</p>
                                    <p class="text-gray-400 text-sm mt-1">Silakan tambah Polda baru</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        <!-- Pagination Footer -->
        @if ($regionalPolices->hasPages())
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <!-- Info Text -->
                    <div class="text-sm text-gray-600">
                        Menampilkan <span class="font-semibold">{{ $regionalPolices->firstItem() }}</span>
                        sampai <span class="font-semibold">{{ $regionalPolices->lastItem() }}</span>
                        dari <span class="font-semibold">{{ $regionalPolices->total() }}</span> hasil
                    </div>

                    <!-- Pagination Numbers -->
                    <div class="flex items-center gap-1">
                        {{-- Previous Button --}}
                        @if ($regionalPolices->onFirstPage())
                        ...
                        {{-- Page Numbers --}}
                        @php
                            $currentPage = $regionalPolices->currentPage();
                            $lastPage = $regionalPolices->lastPage();
                        ...
                        {{-- Next Button --}}
                        @if ($regionalPolices->hasMorePages())
        ...
        @else
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div class="text-sm text-gray-600">
                    Menampilkan <span class="font-semibold">{{ $regionalPolices->count() }}</span> hasil
                </div>
            </div>
        @endif
    </div>
    ...
    <!-- Create/Edit Modal -->
    @include('livewire.admin.master.regional-police.admin-master-regional-police-modal')
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    ...
                    <div class="text-center">
                        ...
                        <h3 class="text-xl font-bold text-gray-900 mb-2">Hapus Polda</h3>
                        <p class="text-gray-500">Apakah Anda yakin ingin menghapus Polda ini? Tindakan ini tidak dapat
                            dibatalkan.</p>
                    </div>
