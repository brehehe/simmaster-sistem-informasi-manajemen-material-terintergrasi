<div>
    ...
    <div class="mb-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-gray-500 mt-1">Kelola data rak penyimpanan</p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
                    ...
                    @forelse ($racks as $index => $rack)
                        <tr class="hover:bg-blue-50/50 transition-colors duration-150">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                {{ $racks->firstItem() + $index }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="font-medium text-gray-900">{{ $rack->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="font-medium text-gray-900">{{ $rack?->regionalPolice?->name ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="font-medium text-gray-900">{{ $rack?->policeStation?->name ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">
                                <div class="max-w-xs truncate">{{ $rack->description ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if ($rack->is_active)
                                    <span
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                {{ $rack->created_at->format('d M Y, H:i') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button wire:click="openEditModal('{{ $rack->id }}')"
                                    ...
                                    <button wire:click="openDeleteModal('{{ $rack->id }}')"
                        ...
                        <tr>
                            <td colspan="6" class="p-10 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 text-lg font-medium">Tidak ada data rak</p>
                                    <p class="text-gray-400 text-sm mt-1">Silakan tambah rak baru</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
            ...
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <div class="text-sm text-gray-600">
                        Menampilkan <span class="font-semibold">{{ $racks->firstItem() }}</span>
                        sampai <span class="font-semibold">{{ $racks->lastItem() }}</span>
                        dari <span class="font-semibold">{{ $racks->total() }}</span> hasil
                    </div>

                    <!-- Pagination Numbers -->
                    <div class="flex items-center gap-1">
                        {{-- Previous Button --}}
                        @if ($racks->onFirstPage())
        ...
        @else
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div class="text-sm text-gray-600">
                    Menampilkan <span class="font-semibold">{{ $racks->count() }}</span> hasil
                </div>
            </div>
        @endif
    </div>
    ...
    <!-- Create/Edit Modal -->
    @include('livewire.admin.master.rack.admin-master-rack-modal')
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    ...
                    <div class="text-center">
                        ...
                        <h3 class="text-xl font-bold text-gray-900 mb-2">Hapus Rak</h3>
                        <p class="text-gray-500">Apakah Anda yakin ingin menghapus rak ini? Tindakan ini tidak dapat
                            dibatalkan.</p>
                    </div>
