<div>
    ...
    <div class="mb-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-gray-500 mt-1">Kelola data tipe pengguna sistem</p>
            </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200">
                        ...
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Types
                        </th>
                    ...
                    @forelse ($userTypes as $index => $userType)
                        <tr class="hover:bg-blue-50/50 transition-colors duration-150">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                {{ $userTypes->firstItem() + $index }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="font-medium text-gray-900">{{ $userType->name }}</div>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600">
                                <div class="max-w-xs truncate">{{ $userType->description ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-wrap gap-1">
                                    @php
                                        $typeNames = $this->getTypeNames($userType->types ?? []);
                                    @endphp
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <button wire:click="openEditModal('{{ $userType->id }}')"
                                    ...
                                    <button wire:click="openDeleteModal('{{ $userType->id }}')"
                        ...
                        <tr>
                            <td colspan="7" class="p-10 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 text-lg font-medium">Tidak ada data tipe user</p>
                                    <p class="text-gray-400 text-sm mt-1">Silakan tambah tipe user baru</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
            ...
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <div class="text-sm text-gray-600">
                        Menampilkan <span class="font-semibold">{{ $userTypes->firstItem() }}</span>
                        sampai <span class="font-semibold">{{ $userTypes->lastItem() }}</span>
                        dari <span class="font-semibold">{{ $userTypes->total() }}</span> hasil
                    </div>

                    <!-- Pagination Numbers -->
                    <div class="flex items-center gap-1">
                        {{-- Previous Button --}}
                        @if ($userTypes->onFirstPage())
        ...
        @else
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div class="text-sm text-gray-600">
                    Menampilkan <span class="font-semibold">{{ $userTypes->count() }}</span> hasil
                </div>
            </div>
        @endif
    </div>
    ...
    <!-- Create/Edit Modal -->
    @include('livewire.admin.master.user-type.admin-master-user-type-modal')
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    ...
                    <div class="text-center">
                        ...
                        <h3 class="text-xl font-bold text-gray-900 mb-2">Hapus Tipe User</h3>
                        <p class="text-gray-500">Apakah Anda yakin ingin menghapus tipe user ini? Tindakan ini tidak
                            dapat
                            dibatalkan.</p>
                    </div>
