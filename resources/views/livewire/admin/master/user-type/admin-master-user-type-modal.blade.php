            ...
            <div class="bg-gradient-to-r from-blue-600 to-cyan-500 px-5 py-3">
                <div class="flex items-center justify-between">
                    <h3 class="text-lg font-bold text-white">
                        {{ $isEditMode ? 'Edit Tipe User' : 'Tambah Tipe User Baru' }}
                    </h3>
            ...
            <form wire:submit="save" class="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
                ...
                <div>
                    ...
                    <textarea wire:model="description" rows="3"
                        class="w-full px-3 py-2 text-sm rounded-lg border {{ $errors->has('description') ? 'border-red-300 focus:border-red-500 focus:ring-red-500/20' : 'border-gray-200 focus:border-blue-500 focus:ring-blue-500/20' }} focus:ring-2 transition-all bg-gray-50 focus:bg-white resize-none"
                        placeholder="Deskripsi tipe user (opsional)"></textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-500">{{ $message }}</p>
                    @enderror
                </div>

                <!-- Types (Multiple Select with Selectize) -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">
                        Types
                    </label>
                    <div wire:ignore wire:key="select-types-{{ $userTypeId ?? 'new' }}">
                        <select id="select-types" multiple x-data x-ref="input" x-init="$($refs.input).selectize({
                            dropdownParent: 'body',
                            plugins: ['remove_button'],
                            delimiter: ',',
                            persist: false,
                            onChange: function(values) {
                                @this.set('types', values ? values : []);
                            }
                        });">
                            @foreach ($allTypes as $type)
                                <option value="{{ $type->id }}" {{ in_array($type->id, $types) ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <label class="relative inline-flex items-center cursor-pointer">
                            ...
                            <div
                                class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600">
                            </div>
                        </label>
