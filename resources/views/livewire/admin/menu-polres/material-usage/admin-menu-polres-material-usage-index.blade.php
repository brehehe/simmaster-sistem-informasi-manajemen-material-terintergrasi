<div>
    ...
    <div class="mb-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <a href="{{ route('menu-polres.material-usage.create') }}" 
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse ($materialUsages as $index => $usage)
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            <td class="px-6 py-4 text-sm text-gray-600 whitespace-nowrap">{{ $materialUsages->firstItem() + $index }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="text-sm font-medium text-blue-600">{{ $usage->materialUsage->code ?? '-' }}</span>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-600 whitespace-nowrap">
                                {{ \Carbon\Carbon::parse($usage->materialUsage->date)->format('d M Y') }}
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-900 whitespace-nowrap">
                                @if($usage->item_code)
                                    <span class="text-xs text-gray-500 mb-1">{{ $usage->item_code ?? '' }}</span>
                                @endif
                                <span>{{ $usage->number_serial_first ?? '' }}</span>
                                @if($usage->number_serial_second)
                                    <span class="text-xs text-gray-500">{{ $usage->number_serial_second ?? '' }}</span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('menu-polres.material-usage.edit', $usage->material_usage_id) }}" 
                                    ...
                                    <button wire:click="openDeleteModal('{{ $usage->material_usage_id }}')"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center whitespace-nowrap">
                                ...
                                <p class="text-gray-400 text-sm mt-1">Silakan tambahkan data material usage</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <!-- Pagination (same as Rack Assignment) -->
        @if ($materialUsages->hasPages())
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="text-sm text-gray-600">
                        Menampilkan <span class="font-semibold">{{ $materialUsages->firstItem() }}</span>
                        sampai <span class="font-semibold">{{ $materialUsages->lastItem() }}</span>
                        dari <span class="font-semibold">{{ $materialUsages->total() }}</span> hasil
                    </div>
                    <div class="flex items-center gap-1">
                        @if ($materialUsages->onFirstPage())
                        ...
                        @endif
                        @php
                            $currentPage = $materialUsages->currentPage();
                            $lastPage = $materialUsages->lastPage();
                        ...
                        @endif
                        @if ($materialUsages->hasMorePages())
        ...
        @else
            <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/50">
                <div class="text-sm text-gray-600">
                    Menampilkan <span class="font-semibold">{{ $materialUsages->count() }}</span> hasil
                </div>
            </div>
        @endif
    </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div
                    ...
                    <div class="text-center">
                        ...
                        <h3 class="text-xl font-bold text-gray-900 mb-2">Hapus Data Material Usage</h3>
