            ...
            <div>
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <h1 class="text-3xl font-bold text-red-600">
                        {{ $isEditMode ? 'Detail Material Rusak/Hilang' : 'Input Material Rusak/Hilang' }}
                    </h1>
                </div>
                <p class="text-gray-500 ml-14">Data kerusakan atau kehilangan material di Polres</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden mb-6">
        <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-red-50 to-orange-50/50">
            <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                ...
                Informasi Kerusakan/Kehilangan
            </h2>
        </div>
        <div class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Material Utama -->
                <div class="border border-red-100 bg-red-50/50 p-4 rounded-xl">
                    <label class="flex items-center gap-2 text-sm font-semibold text-red-900 mb-2">
                ...
                <div>
                    ...
                    <textarea wire:model="description" rows="2" @disabled($isEditMode) class="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 disabled:bg-gray-100 disabled:text-gray-500" placeholder="Catatan kerusakan/kehilangan material..."></textarea>
                </div>
            </div>
        </div>
    </div>
                ...
                <table class="w-full text-sm text-left align-top border-collapse min-w-[1200px]">
                    <thead class="bg-gray-50 border-b border-gray-200 text-gray-700">
                        <tr>
                            ...
                            @endif
                            <th class="px-3 py-3 font-semibold text-xs min-w-[140px]">Status Barang</th>
                            ...
                            <th class="px-3 py-3 font-semibold text-xs min-w-[160px]">Alasan <span class="text-red-500">*</span></th>
                            @if (!$isEditMode)
                    ...
                    <tbody class="divide-y divide-gray-100">
                            ...
                            <tr wire:key="row-{{ $index }}-{{ $typeId }}" class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-3 py-3 align-top">
                                    <select wire:model.live="details.{{ $index }}.damage_type" @disabled($isEditMode)
                                        class="w-full px-2 py-2 text-xs rounded border border-gray-300 focus:border-red-500 disabled:bg-gray-100 disabled:text-gray-500 {{ $detail['damage_type'] == 'lost' ? 'bg-red-50 text-red-700 font-semibold' : '' }}">
                                ...
                                <td class="px-3 py-3 align-top">
                                    <input type="text" wire:model.defer="details.{{ $index }}.reason" placeholder="Alasan..." @disabled($isEditMode)
                                        class="w-full px-2 py-2 text-xs rounded border border-gray-300 focus:border-red-500 disabled:bg-gray-100 disabled:text-gray-500">
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="py-12 text-center text-gray-400 italic">Klik "Tambah Item" untuk mulai input kerusakan material...</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-end gap-3 mt-4">
        ...
        @if (!$isEditMode)
            <button wire:click="save" type="button"
                ...
                <span wire:loading.remove wire:target="save">💾 Simpan Kerusakan</span>
